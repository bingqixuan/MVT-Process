...
MBTile.read = function (pbf, end) {
    return pbf.readFields(MBTile._readField, {layers: []}, end);
};
MBTile._readField = function (tag, obj, pbf) {
    if (tag === 3) obj.layers.push(MBTile.Layer.read(pbf, pbf.readVarint() + pbf.pos));
};
MBTile.write = function (obj, pbf) {
    if (obj.layers) for (var i = 0; i < obj.layers.length; i++) pbf.writeMessage(3, MBTile.Layer.write, obj.layers[i]);
};
...
MBTile.Value.read = function (pbf, end) {
    return pbf.readFields(MBTile.Value._readField, {
        string_value: "",
        float_value: 0,
        double_value: 0,
        int_value: 0,
...
MBTile.Value._readField = function (tag, obj, pbf) {
...
MBTile.Value.write = function (obj, pbf) {
...
MBTile.Feature.read = function (pbf, end) {
    return pbf.readFields(MBTile.Feature._readField, {id: 0, tags: [], type: 0, geometry: []}, end);
};
MBTile.Feature._readField = function (tag, obj, pbf) {
...
MBTile.Feature.write = function (obj, pbf) {
...
MBTile.Layer.read = function (pbf, end) {
    return pbf.readFields(MBTile.Layer._readField, {
        version: 1,
        name: "",
        features: [],
        keys: [],
...
MBTile.Layer._readField = function (tag, obj, pbf) {
    if (tag === 15) obj.version = pbf.readVarint();
    else if (tag === 1) obj.name = pbf.readString();
    else if (tag === 2) obj.features.push(MBTile.Feature.read(pbf, pbf.readVarint() + pbf.pos));
    else if (tag === 3) obj.keys.push(pbf.readString());
    else if (tag === 4) obj.values.push(MBTile.Value.read(pbf, pbf.readVarint() + pbf.pos));
    else if (tag === 5) obj.extent = pbf.readVarint();
};
MBTile.Layer.write = function (obj, pbf) {
    if (obj.version != undefined && obj.version !== 1) pbf.writeVarintField(15, obj.version);
    if (obj.name) pbf.writeStringField(1, obj.name);
    if (obj.features) for (var i = 0; i < obj.features.length; i++) pbf.writeMessage(2, MBTile.Feature.write, obj.features[i]);
